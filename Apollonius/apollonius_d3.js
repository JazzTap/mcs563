...
// function (d3) {
    ...
    function dragwrap(dragged) { // FIXME: do not snap to (0,0)!
        return d3.behavior.drag()
            ...
            .on("dragend", function(d) {
                  ...
                  done = false // submitState() // pre-realtime update
                })
    }
    ...
    input.selectAll('circle').filter(function (d,i) {return i!=0})
           .on("wheel", function(d,i) {
                // console.log(d)
                console.log(event)
                dy = Math.sign(event.deltaY) // control for values ~1 on touchpad, ~100 on mouse
                d3.select(this).attr("r",
                    d.r = Math.max(.1, d.r - .1*dy)) // .1*event.deltaY
